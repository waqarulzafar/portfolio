...
const services = [
  ...
  { icon: 'i-lucide-store', name: 'Asan POS', description: 'Manage everyday sales and stock with Asan POS, a ready-made point-of-sale system available for sale.', price: '300', label: 'Ready-made solution', features: ['For retail, grocery & restaurants', 'Sales, inventory & customer management', 'Discuss the right fit for your team'], inquiry: 'Asan POS' }
]
const steps = [
...
const faqs = [
  ...
  { label: 'Is Asan POS right for my business?', content: 'Asan POS is offered for $300 for retail, grocery, restaurant, and service businesses. Contact me with your workflow and device requirements so we can confirm suitability, available features, and any setup or customization costs.' },
...
<template>
  <div>
    ...
    <main id="main-content" tabindex="-1">
      ...
      <section v-if="projects.length" id="projects" class="section container projects-section">
        <div class="section-heading reveal">
          <div><span class="eyebrow">01 / SELECTED PROJECTS</span><h2>Ideas, brought to life.</h2></div>
          <p>A marketplace for independent talent.<br>Software for everyday business.</p>
        </div>
        <div class="project-grid">
          <article v-for="project in projects" :key="project.name" class="project-card reveal">
            <a :href="project.image" target="_blank" rel="noopener noreferrer" class="project-image-link" :aria-label="`View full ${project.name} project image (opens in a new tab)`">
            ...
            <div class="project-meta"><span class="eyebrow">{{ project.category }}</span><span v-if="project.forSale" class="project-sale">Available for sale</span></div>
            <h3>{{ project.name }}</h3>
            <p>{{ project.description }}</p>
            <div class="tag-list"><span v-for="tech in project.technologies" :key="tech">{{ tech }}</span></div>
            <a v-if="project.forSale" class="project-inquiry text-link" href="#contact" @click="chooseService('Asan POS')">Inquire about Asan POS <UIcon name="i-lucide-arrow-up-right" /></a>
          </article>
        </div>
      </section>
      ...
      <section id="pos" class="section container pos-section"><div class="pos-panel reveal"><div class="pos-copy"><span class="eyebrow">ASAN POS / AVAILABLE FOR SALE</span><h2>Less setup.<br>More doing.</h2><p>Bring sales, inventory, and customer management together with Asan POS. Let's walk through your daily operations and find out whether it's the right fit.</p><div class="industry-tags"><span><UIcon name="i-lucide-utensils" /> Restaurants</span><span><UIcon name="i-lucide-building-2" /> Retail</span><span><UIcon name="i-lucide-store" /> Grocery</span><span><UIcon name="i-lucide-plus" /> And more</span></div><a class="button" href="#contact" @click="chooseService('Asan POS')">Ask about Asan POS <UIcon name="i-lucide-arrow-up-right" /></a></div><div class="pos-price-panel"><UIcon name="i-lucide-store" /><span>ASAN POS</span><strong><small>$</small>300</strong><span class="pos-currency">USD · Ready-made software</span><div class="pos-divider" /><p>Tell me your business type, required features, and devices. We’ll confirm the details before you commit.</p><a href="https://wa.me/92487347143?text=Hi%20Waqar%2C%20I%27d%20like%20to%20know%20more%20about%20Asan%20POS." target="_blank" rel="noopener noreferrer">Let's discuss the fit <UIcon name="i-lucide-arrow-up-right" /></a></div></div></section>
